...
class ConformalPredictor:
    ...
    # -------------------------------------------
    # 4. Hybrid Conformalized Quantile Regression (CQR) with Adaptive Conformal Inference (ACI)
    # -------------------------------------------
    def cqr_aci(self, pred_lower_calib, pred_upper_calib, y_calib,
                pred_lower_test, pred_upper_test, y_test, gamma=0.005):
        """
        Hybrid method combining Conformalized Quantile Regression (CQR) with 
        Adaptive Conformal Inference (ACI). It adapts the conformal correction
        dynamically based on past prediction success.
        ...
        pred_upper_test : np.ndarray
            Predicted upper quantiles on the test set.
        y_test : np.ndarray
            True labels/targets for the test set (streaming/temporal order).
        gamma : float
            Adaptation rate for the correction (default = 0.005).

        Returns
        -------
        pd.DataFrame
            DataFrame with dynamically adjusted prediction intervals and violation indicators.
        """
        # Step 1: Compute initial calibration nonconformity scores
        calib_scores = np.maximum(pred_lower_calib - y_calib, y_calib - pred_upper_calib)
        calib_scores = np.sort(calib_scores)

        # Step 2: Initialize adaptive alpha and output containers
        adaptive_alpha = self.alpha
        lower_bounds = []
        upper_bounds = []
        y_trues = []
        ...
        # Ensure arrays
        pred_lower_test = np.array(pred_lower_test)
        pred_upper_test = np.array(pred_upper_test)
        y_test = np.array(y_test)

        # Step 3: Loop over test points
        for i in range(len(y_test)):
            # Step 3.1: Determine quantile for adaptive alpha
            q_hat = np.quantile(calib_scores, 1 - adaptive_alpha, method='higher')

            # Step 3.2: Adjust model-predicted quantiles
            lower = pred_lower_test[i] - q_hat
            upper = pred_upper_test[i] + q_hat

            # Step 3.3: Check for violation
            violation = int((y_test[i] < lower) or (y_test[i] > upper))

            # Step 3.4: Update adaptive alpha
            adaptive_alpha += gamma * (self.alpha - violation)
            adaptive_alpha = np.clip(adaptive_alpha, 0, 1)

            # Step 3.5: Store results
            lower_bounds.append(lower)
            upper_bounds.append(upper)
            y_trues.append(y_test[i])
            violations.append(violation)

        # Step 4: Format final DataFrame
        return self._make_final_dataframe(
            np.array(lower_bounds),
            np.array(upper_bounds),
            np.array(y_trues),
